<?php
jsou tu některé metody, které je ještě třeba implementovat
/**
...
class Projektor_Model_Zajemce extends Projektor_Model_HlavniObjekt
{
...
//TODO: přejmenovat sloupec v db na id_zajemce_FK a změnit konstantu
    ...
    public static function najdiPodleId($id) {
        return parent::najdiPodleId(self::DATABAZE, $id, __CLASS__, self::TABULKA, self::ID, self::CISLO_OBJEKTU, FALSE);
    }

    public static function vypisVse($filtr = "", $orderBy = "", $order = "") {
        return parent::vypisVse(self::DATABAZE, $filtr, $orderBy, $order, __CLASS__, self::TABULKA, self::ID, self::CISLO_OBJEKTU, FALSE);
    }
...
//        $seznamisco = Projektor_Model_Seznam_SISCO::vypisVse("LENGTH(`".Projektor_Model_Seznam_SISCO::KOD."`)=".$delkaKodu." AND LEFT(`".Projektor_Model_Seznam_SISCO::KOD."`, ".$delkaPrefixu.")='".$prefix."'");
...
//                ;
        ...
        $filtr = "LENGTH(`KZAM_cislo1`)=5".
                ...
                ;
        $dotazniky = Projektor_Model_FlatTable::vypisVse(self::DATABAZE, $tabulka, $filtr, "", "", $this, "", "", "", TRUE);

        $vhodniZajemci = array();
        foreach ($dotazniky as $dotaznik) {
            $zajemce = parent::najdiPodleId(self::DATABAZE, $dotaznik->id, __CLASS__, self::TABULKA, self::ID, self::CISLO_OBJEKTU, FALSE);// pro nevalidní zájemce vrací FALSE
            if ($zajemce)
            {
    ...
    {
        //TODO: tady v Projektor_Model_Zajemce jen nastav filtr a volej Projektor_Model_HlavniObjekt::vypisVse, v Projektor_HlavniObjekt zruš metodu vypisPrihlaseneNaAkci
        return parent::vypisPrihlaseneNaAkci(self::DATABAZE, $idAkce, $filtr, $orderBy, $order, __CLASS__, self::TABULKA, self::ID, self::CISLO_OBJEKTU, FALSE);
    }
    ...
    private static function predpoklady($iscoKod, $dotaznik)
        {
            ...
            $kzamKvalikacniPredpoklady = Projektor_Model_FlatTable::vypisVse(self::DATABAZE, "s_kzam_kvalifikacni_predpoklady", $filtr, "", "", NULL, "", "", "", TRUE);
            $predpoklad = 0;
            for ($i = 1; $i <= 5; $i++) {
