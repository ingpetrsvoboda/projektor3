...
/**
 * Description of Projektor_Model_Auto_Autocode_Koder
 *
 * @author pes2704
 */
class Projektor_Model_Auto_Autocode_Koder {
...
// hodnoty nařítané z konfigurace aplikace ve třídě Framework_Config
    /**
    ...
    private function __construct($phpFullFileName, $finalize, $autocodeConfig, $logDirectoryPath) {
        ...
        $this->logger = Projektor_Model_Auto_Autocode_Logger::getInstance($logDirectoryPath, $phpFileName.".log");
        ...
        if ($this->getContent()){
            if ($this->findAutocodeSlot()) {
                ...
                if ($this->className){
                    $maKonstanty = TRUE;
                    if (is_subclass_of($this->class, "Projektor_Model_Item")) {
                    ...
                    if (is_subclass_of($this->class, "Projektor_Model_Collection")) {
    ...
    /**
     * Metoda vrací instanci objektu třídy Projektor_Model_Auto_Autocode_Koder. Objekt Projektor_Model_Auto_Autocode_Koder je vytvářen jako singleton pro jeden php soubor.
     ...
     * @return Projektor_Model_Auto_Autocode_Koder
     */
...
//                                        $this->logger->loguj(self::ODSAZENI."***********");
    ...
    private function dejAutocodeItem()
    {
        ...
        if (is_subclass_of($this->class, "Projektor_Model_HlavniObjektItem")) { //class definuje hlavní objekt - je potomek Projektor_Model_HlavniObjektItem
        ...
        if (!$this->finalize) {
            $strukturaTabulky = Projektor_Model_Auto_Cache_Struktury::getStrukturuTabulky($databaze, $tabulka);
            ...
            foreach ($strukturaTabulky->sloupce as $strukturaSloupce) {
                $autocodeDB .= "    /**\n";
                $autocodeDB .= "     * Generovaná vlastnost pro tabulku ".$tabulka." a sloupec ".$strukturaSloupce->controllerName.". Vlatnosti sloupce: typ=".$strukturaSloupce->typ;
                ...
                // nahrazeni mezer v názvu sloupce db znaky '°'
                $nazev = str_replace(' ', '°', $strukturaSloupce->controllerName);
                ...
                case "PK":
                    ...
                    if ($strukturaSloupce->extra=="auto_increment") {
                        ...
                        $vlastnostDBField = $this->autocodeConfig->dbfieldprefix.$nazev;
                    }
                    break;
                case "FK":
                    $autocodeDB .= "\n";
...
//                $autocodeDB .= "    public static $".$vlastnostDBField.";\n";
            ...
            if (isset($poleMapovani)) {
                foreach ($poleMapovani as $vlastnost => $itemClass) {
                    ...
                    $autocodeMetody .="    /**\n";
                    $autocodeMetody .="     * Metoda vrací vlastnost hlavního objektu ". ucfirst($vlastnost)." typu ".$itemClass."\n";
                    $autocodeMetody .="     * @param ".$itemClass." \$object Tento parametr při volání metody nikdy NEZADÁVEJTE. Jedná se pouze o hack pro type hinting.\n";
        ...
        if (isset($poleMapovani)) {
            ...
            $autocodeGetSet .="        }\n";
            $autocodeGetSet .="        return parent::__set(\$nazevVlastnosti, \$value);\n";
            $autocodeGetSet .="    }\n";
            $autocodeGetSet .="\n";
        }
        if (!$this->finalize) {
...
//                    is_callable????
        ...
        if (!$this->finalize) {
            ...
            $autocode .= "\n";
            $autocode .= "    /**\n";
            $autocode .= "     * Factory metoda provádí type hinting pro našeptávání v IDE.\n";
            $autocode .= "     * Metoda vrací objekt typu ".$itemClassName.", prvek kolekce ".$className.".\n";
            $autocode .= "     * @param \$id \$object Tento parametr při volání metody zadávejte.\n";
            $autocode .= "     * @param ".$itemClassName." \$object Tento parametr při volání metody nikdy NEZADÁVEJTE. Jedná se pouze o hack pro type hinting.\n";
