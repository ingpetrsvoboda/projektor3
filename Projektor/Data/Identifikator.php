<?php
class Projektor_Data_Identifikator {
	...
	public function generuj($cislo_kontextu = NULL, $cislo_rozsahu = NULL, $cislo_projektu = NULL, $cislo_behu = null, $cislo_polozky = NULL){
		...
		if($cislo_projektu &&  $cislo_rozsahu && $cislo_kontextu && $cislo_behu && $cislo_polozky) {
                    $retez = "";
                    if (Projektor_Data_Identifikator::pridejCislo($retez, $cislo_kontextu,1)){
                            if (Projektor_Data_Identifikator::pridejCislo($retez, $cislo_rozsahu,2)){
                                if (Projektor_Data_Identifikator::pridejCislo($retez, $cislo_projektu,2)){
                                    if (Projektor_Data_Identifikator::pridejCislo($retez, $cislo_behu,2)){
                                        if (Projektor_Data_Identifikator::pridejCislo($retez, $cislo_polozky,3)){
                                            return new Projektor_Data_Identifikator($cislo_kontextu, $cislo_rozsahu, $cislo_projektu, $cislo_behu, $cislo_polozky, $retez);
                                        }
                                    }
                                }
                            }
