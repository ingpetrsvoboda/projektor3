...
//		$titulek->Text('Lákamí vůněhulás úmyval rohlivý jednovod lek lák hane bývá přehliv smeti. Smělý Umyslemi dopicí sudba rojskočár ří bý autný tlínům z zavěď. Umí jít A hafan bý obal stako tak úmyvatkov Buben muto. ');
    ...
    $ukonceniUcasti = new Projektor_Pdf_SadaBunek();
	...
	$ukonceniUcasti->PridejBunku("Důvod ukončení účasti v projektu: ", $duvod_ukonceni_pole[0],1);  
	    ...
	    $ukonceniUcasti1 = new Projektor_Pdf_Odstavec;
	    $ukonceniUcasti1->text( @$pdfpole['popis_ukonceni']);
	}
	
    $poznKUkonceni = new Projektor_Pdf_Odstavec;
		$poznKUkonceni->Text("Možné důvody:");
		$poznKUkonceni->VyskaPismaTextu(8);
	$poznKUkonceni1 = new Projektor_Pdf_Odstavec;
	...
	$poznKUkonceni1a = new Projektor_Pdf_Odstavec;
		$poznKUkonceni1a->Text("a. běžně v době 3 měsíce");
		$poznKUkonceni1a->VyskaPismaTextu(8);
		$poznKUkonceni1a->OdsazeniZleva(3);
        $poznKUkonceni1a->Predsazeni(3);
	$poznKUkonceni1b = new Projektor_Pdf_Odstavec;
		$poznKUkonceni1b->Text("b. v případě účasti klienta v profesním rekvalifikačním kurzu (tedy nikoli v kurzech Obsluha osobního počítače nebo Obsluha osobního počítače dle osnov ECDL START) nebo na praxi končí jeho účast po uplynutí 14 dní od absolvování kurzu, pokud je tato doba delší než 3 měsíce");
		$poznKUkonceni1b->VyskaPismaTextu(8);
        $poznKUkonceni1b->OdsazeniZleva(3);
        $poznKUkonceni1b->Predsazeni(3);
    $poznKUkonceni2 = new Projektor_Pdf_Odstavec;
		$poznKUkonceni2->Text("2. předčasným ukončením účasti ze strany klienta");
		$poznKUkonceni2->VyskaPismaTextu(8);
	$poznKUkonceni2a = new Projektor_Pdf_Odstavec;
    ...
    $poznKUkonceni2b = new Projektor_Pdf_Odstavec;
		$poznKUkonceni2b->Text("b. výpovědí dohody o účasti v projektu klientem z jiného důvodu než nástupu do zaměstnání (ukončení dnem, kdy byla výpověď doručena zástupci dodavatele) ");
		$poznKUkonceni2b->VyskaPismaTextu(8);
        $poznKUkonceni2b->OdsazeniZleva(3);
        $poznKUkonceni2b->Predsazeni(3);
    $poznKUkonceni3 = new Projektor_Pdf_Odstavec;
		$poznKUkonceni3->Text("3. předčasným ukončením účasti ze strany dodavatele");
		$poznKUkonceni3->VyskaPismaTextu(8);
	$poznKUkonceni3a = new Projektor_Pdf_Odstavec;
    ...
    $poznKUkonceni3b = new Projektor_Pdf_Odstavec;
		$poznKUkonceni3b->Text("b. ve výjimečných případech na základě podnětu vysílajícího ÚP (např. při sankčním vyřazení z evidence ÚP)");
		$poznKUkonceni3b->VyskaPismaTextu(8);
        $poznKUkonceni3b->OdsazeniZleva(3);
        $poznKUkonceni3b->Predsazeni(3);
        
    $osvedceni = new Projektor_Pdf_SadaBunek();
        ...
        $osvedceni->NovyRadek();
	$poznamkaOsvedceni = new Projektor_Pdf_Odstavec;
		$poznamkaOsvedceni->Text("Po ukončení účasti klienta v projektu řádným způsobem nebo z důvodu nástupu do zaměstnání po absolvování alespoň 3 aktivit projektu získá účastník Osvědčení o absolvování projektu S pomocí za prací.");
...
*/
    $vyhodnoceni=new Projektor_Pdf_SadaBunek();
    $vyhodnoceni->Nadpis("Vyhodnocení");
    $vyhodnoceniMot = new Projektor_Pdf_Odstavec;
    $vyhodnoceniMot->Text(@$pdfpole['mot_hodnoceni']);
    $vyhodnoceniPC1 = new Projektor_Pdf_Odstavec;
    $vyhodnoceniPC1->Text(@$pdfpole['pc1_hodnoceni']);
    $vyhodnoceniPC2 = new Projektor_Pdf_Odstavec;
    $vyhodnoceniPC2->Text(@$pdfpole['pc2_hodnoceni']);
    $vyhodnoceniBidi = new Projektor_Pdf_Odstavec;
    $vyhodnoceniBidi->Text(@$pdfpole['bidi_hodnoceni']);
    $vyhodnoceniPrdi = new Projektor_Pdf_Odstavec;
    $vyhodnoceniPrdi->Text(@$pdfpole['prdi_hodnoceni']);
    /*$vyhodnoceniPraxe = new Projektor_Pdf_Odstavec;
    $vyhodnoceniPraxe->Text(@$pdfpole['praxe_hodnoceni']);*/
    $vyhodnoceniProf1 = new Projektor_Pdf_Odstavec;
    $vyhodnoceniProf1->Text(@$pdfpole['prof1_hodnoceni']);
    $vyhodnoceniProf2 = new Projektor_Pdf_Odstavec;
    $vyhodnoceniProf2->Text(@$pdfpole['prof2_hodnoceni']);
    $vyhodnoceniProf3 = new Projektor_Pdf_Odstavec;
    $vyhodnoceniProf3->Text(@$pdfpole['prof3_hodnoceni']);
    $vyhodnoceniPoradenstvi = new Projektor_Pdf_Odstavec;
    $vyhodnoceniPoradenstvi->Text(@$pdfpole['porad_hodnoceni']);
    $vyhodnoceniDoporuceni = new Projektor_Pdf_Odstavec;
    $vyhodnoceniDoporuceni->Text(@$pdfpole['doporuceni']);
    $vyhodnoceniDalsi = new Projektor_Pdf_Odstavec;
    ...
    $priloha = new  Projektor_Pdf_SadaBunek;                //Projektor_Pdf_Odstavec;
            $priloha->PridejBunku("Příloha: ", " ",1);
    $priloha1 =  new Projektor_Pdf_Odstavec;
