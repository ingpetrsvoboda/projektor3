<?php
class Projektor_Stranka_Element_Hlavicka
{
	public $sloupce;
        private $nazevTridyDataItem;
	private $uzel;

	public function __construct($nazevTridyDataItem, Projektor_Dispatcher_Uzel $uzel)
	{
        ...
        public function pridejSloupec($nazevVlastnosti, $titulek=NULL, $nazevVlastnostiReferencovanehoObjektu = NULL)
	{
            ...
            if ($nazevSloupceDb)
            {
                ...
                $this->sloupce[] = new Projektor_Stranka_Element_Hlavicka_Sloupec
                (
                    $nazevVlastnosti,
                    $titulek,
                    $nazevVlastnostiReferencovanehoObjektu,
                    $this->uzel->setParametr("razeniPodle", $nazevSloupceDb)->setParametr("razeni", "DESC")->formAction(),
                    $razeniDescPouzito,
                    $this->uzel->setParametr("razeniPodle", $nazevSloupceDb)->setParametr("razeni", "ASC")->formAction(),
                    $razeniAscPouzito
                );
                //vrácení hodnot parametrů
                $this->uzel->setParametr("razeniPodle", $razeniPodle)->setParametr("razeni", $razeni)->formAction();
            } else {
