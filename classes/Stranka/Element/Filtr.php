...
class Stranka_Element_Filtr
{
    ...
    {
        $filtr = new Stranka_Element_Filtr();
        foreach($sloupceHodnoty as $sloupec => $hodnota)
            if($hodnota)
                $filtr->pravidla[] = new Stranka_Element_Filtr_Pravidlo($sloupec, Stranka_Element_Filtr::EQUALS, $hodnota);
    ...
    public static function like($sloupceHodnoty = NULL)
    {
        ...
        if ($sloupceHodnoty) {
            foreach($sloupceHodnoty as $sloupec => $hodnota)
            if($hodnota)
                $filtr->pravidla[] = new Stranka_Element_Filtr_Pravidlo($sloupec, Stranka_Element_Filtr::LIKE, $hodnota, true, true);            
        }
