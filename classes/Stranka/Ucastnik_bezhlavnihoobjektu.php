...
class Stranka_Ucastnik extends Stranka implements Stranka_Interface
{
	...
	public function detail($parametry = null)
	{
		...
		$form = new HTML_QuickForm("ucastnik", "post", $this->cestaSem->generujUri());
		$ucastnik = Data_Ucastnik::najdiPodleId($parametry["id"]);
                    ...
                    foreach ($nazvy as $key => $jmenoVlatnosti) {
                        ...
                        if ($klice[$key]) {             //elementy, které odpovídají sloupcům db tabulky obsahujícím klíče musí být hidden nebo static
                            ...
                            if ($typy[$key] == "date") {
                                ...
                                $elementy["default"][$index] = Data_Konverze_Datum::zSQL($objektVlastnost->$jmenoVlatnosti)->dejDatumProQuickForm() ;
                            } else {
...
//                    if (!array_key_exists("ucastnik_id", $data)) $ucastnik = new Data_Ucastnik(0, 0, $idSBehProjektuFK, $idCKancelarFK ); //tyto parametry musí být povinne vyplneny ve formulari
		...
		if($form->validate())
		{
                    ...
                    if ($ucastnik) {
                        ...
                        $ucastnikZFormulare = Data_Ucastnik::najdiPodleId($parametry["id"]);
                    } else {
                        // nový účastník
                        $ucastnikZFormulare = new Data_Ucastnik($data["cisloUcastnika"], $data["identifikator"], $data["idSBehProjektuFK"], $data["idCKancelarFK"], 1, $data["id"]);
