...
class Data_Seznam_SAkcePredpoklad extends Data_Iterator
{
	...
	public function __construct($text, $fullText, $idSTypAkceFK, $idSTypAkcePredFK, $idSStavUcastnikAkcePredFK, $id = null)
	{
                 $this->id = $id;
                 $this->text = $text;
	...
	public static function najdiPodleId($id)
	{
		...
		$radek[self::ID_S_STAV_UCASTNIK_AKCE_PRED_FK], $radek[self::ID]);
	}
	...
	public function uloz()
	{
		...
		if($this->id == null)
		{
			...
			self::TABULKA, self::TEXT, self::FULL_TEXT, self::ID_S_TYP_AKCE_FK, self::ID_S_TYP_AKCE_PRED_FK, self::ID_S_STAV_UCASTNIK_AKCE_PRED_FK,
			$this->text, $this->fullText, $this->idSTypAkceFK, $this->idSTypAkcePredFK, $this->idSStavUcastnikAkcePredFK
			)->last_insert_id();
		}
		else
		{
